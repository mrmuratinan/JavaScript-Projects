				...
				function (_dereq_, module, exports) {
					...
					function mixSpecIntoComponent(Constructor, spec) {
						...
						!(typeof spec !== 'function')
							? 'development' !== 'production'
								? invariant(false, "ReactClass: You're attempting to " + 'use a component class as a mixin. Instead, just use a regular object.')
								: invariant(false)
							: undefined;
							...
							if (voidElementTags[component._tag]) {
								'development' !== 'production'
									? warning(
											...
											component._tag,
											component._currentElement._owner ? ' Check the render method of ' + component._currentElement._owner.getName() + '.' : ''
									  )
									: undefined;
							}
						}
				...
				function (_dereq_, module, exports) {
					...
					function getNextDescendantID(ancestorID, destinationID) {
						...
						!isAncestorIDOf(ancestorID, destinationID)
							? 'development' !== 'production'
								? invariant(false, 'getNextDescendantID(...): React has made an invalid assumption about ' + 'the DOM hierarchy. Expected `%s` to be an ancestor of `%s`.', ancestorID, destinationID)
								: invariant(false)
							: undefined;
					...
					function traverseParentPath(start, stop, cb, arg, skipFirst, skipLast) {
						...
						!(traverseUp || isAncestorIDOf(start, stop))
							? 'development' !== 'production'
								? invariant(false, 'traverseParentPath(%s, %s, ...): Cannot traverse from two IDs that do ' + 'not have a parent path.', start, stop)
								: invariant(false)
							: undefined;
						...
						for (var id = start; ; /* until break */ id = traverse(id, stop)) {
							...
							!(depth++ < MAX_TREE_DEPTH)
								? 'development' !== 'production'
									? invariant(false, 'traverseParentPath(%s, %s, ...): Detected an infinite loop while ' + 'traversing the React DOM ID tree. This may be due to malformed IDs: %s', start, stop, id)
									: invariant(false)
								: undefined;
						}
					}
				...
				function (_dereq_, module, exports) {
					...
					var ReactMount = {
						...
						_registerComponent: function (nextComponent, container) {
							!(container && (container.nodeType === ELEMENT_NODE_TYPE || container.nodeType === DOC_NODE_TYPE || container.nodeType === DOCUMENT_FRAGMENT_NODE_TYPE))
								? 'development' !== 'production'
					...
					function createPrimitiveTypeChecker(expectedType) {
						function validate(props, propName, componentName, location, propFullName) {
							...
							if (propType !== expectedType) {
								...
								return new Error(
									'Invalid ' + locationName + ' `' + propFullName + '` of type ' + ('`' + preciseType + '` supplied to `' + componentName + '`, expected ') + ('`' + expectedType + '`.')
								);
							}
							return null;
						}
					...
					var ReactUpdateQueue = {
						...
						enqueueCallback: function (publicInstance, callback) {
							!(typeof callback === 'function')
								? 'development' !== 'production'
									? invariant(false, 'enqueueCallback(...): You called `setProps`, `replaceProps`, ' + '`setState`, `replaceState`, or `forceUpdate` with a callback that ' + "isn't callable.")
									: invariant(false)
								: undefined;
				...
				function (_dereq_, module, exports) {
					...
					function runBatchedUpdates(transaction) {
						...
						!(len === dirtyComponents.length)
							? 'development' !== 'production'
								? invariant(false, "Expected flush transaction's stored dirty-components length (%s) to " + 'match dirty-components array length (%s).', len, dirtyComponents.length)
								: invariant(false)
							: undefined;
					...
					function asap(callback, context) {
						!batchingStrategy.isBatchingUpdates
							? 'development' !== 'production'
								? invariant(false, "ReactUpdates.asap: Can't enqueue an asap callback in a context where" + 'updates are not being batched.')
								: invariant(false)
							: undefined;
					...
					var Mixin = {
						...
						perform: function (method, scope, a, b, c, d, e, f) {
							!!this.isInTransaction()
								? 'development' !== 'production'
									? invariant(false, 'Transaction.perform(...): Cannot initialize a transaction when there ' + 'is already an outstanding transaction.')
									: invariant(false)
								: undefined;
							var errorThrown;
							var ret;
